...
\texttt{/users/nelle}, which is Nelle's \gl{home
...
If the command to find out who we are is \texttt{whoami}, why is the
command to find out where we are called \texttt{pwd} instead of
\texttt{whereami}?  The usual answer is that in the early 1970s, when
...
the leading slash in \texttt{/users/nelle}.
...
We know that our current working directory \texttt{/users/nelle} is
...
Underneath \texttt{/users}, we find one directory for each user with
an account on this machine (\figref{f:homedir}). The Mummy's files are
stored in \texttt{/users/imhotep}, Wolfman's in \texttt{/users/larry},
and ours in \texttt{/users/nelle}, which is why \texttt{nelle} is the
...
Let's see what's in Nelle's home directory by running \texttt{ls}, which
...
Here, we can see that \texttt{/users/nelle} contains seven
...
PNG image of a whale as \texttt{whale.mp3} doesn't somehow magically
...
Now let's take a look at what's in Nelle's \texttt{data} directory by
running \texttt{ls -F data}, i.e., the command \texttt{ls} with the
arguments \texttt{-F} and \texttt{data}. The second argument---the one
...
this, \texttt{pwd} shows us that we're in \texttt{/users/nelle}, and
...
we can see that we are now in \texttt{/users/nelle/data}. If we run
\texttt{ls} without arguments now, it lists the contents of
\texttt{/users/nelle/data}, because that's where we now are:
...
(which, if we're in \texttt{/users/nelle}, refers to the \texttt{/users}
...
example, if we are in \texttt{/users/nelle/data}, the command
\texttt{ls ..} will give us a listing of \texttt{/users/nelle}. When the
...
\texttt{/users/nelle}, and use \texttt{ls -F} to see what it contains:
...
shorthand \texttt{\^{}A} to mean ``control-A''.) \texttt{nano} doesn't
leave any output on the screen after it exits, but \texttt{ls} now
shows that we have created a file called \texttt{draft.txt}:
...
\texttt{/users/nelle} using \texttt{cd ..}:
...
\noindent
The first argument tells \texttt{mv} what we're ``moving'', while the
...
directory as the second argument to tell \texttt{mv} that we want to
...
\noindent
Further, \texttt{ls} with a filename or directory name as an argument
...
thing using \texttt{ls} with two paths as arguments---like most Unix
...
example, if Nelle's home directory is \texttt{/home/nelle}, then
\texttt{\textasciitilde{}/data} expands to
\texttt{/home/nelle/data}. This only works if it is the first character
in the path: \texttt{here/there/\textasciitilde{}/elsewhere} is
\emph{not} \texttt{/home/nelle/elsewhere}.
...
Using the argument \texttt{-1} with \texttt{head} tells it that we only
...
Since we've provided some filenames as arguments, \texttt{wc} reads
...
\texttt{wc} doesn't have any command line arguments, so it reads from
...
\texttt{\$*}, which means, ``All of the command-line arguments to the
...
\noindent
but don't add \texttt{*.dat} (or anything else)? In this case,
...
should have provided a couple of extra arguments to \texttt{goostats}
...
\noindent
(The arguments \texttt{-J 100} and \texttt{-r} are the ones her
...
command-line arguments, and use \texttt{*{[}AB{]}.txt} if none were
...
\end{VerbFile}
  ...
  of filenames as command-line arguments, and uses \texttt{cut},
...
Nelle's home directory contains one file called \texttt{notes.txt} and
